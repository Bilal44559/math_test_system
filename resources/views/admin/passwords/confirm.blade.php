...
<div class="authentication-wrapper authentication-basic row my-5">
    <div class="authentication-inner col-lg-8 mx-auto">
        <div class="card shadow-sm rounded border-0 px-4">
            <div class="card-body p-4">
                ...
                <form id="otpForm" method="POST" action="{{ route('admin.verifyOtpAdmin') }}">
                    @csrf
                    <input type="hidden" name="email" value="{{ $email }}">

                    <div class="mb-4">
                        <h6 for="otp" class="form-label text-center w-100">OTP Code</h6>
                        <input type="text" id="otp"
                            class="form-control form-control-lg text-center fs-3 py-2 @error('otp') is-invalid @enderror"
                            name="otp" placeholder="------" autofocus maxlength="6" inputmode="numeric" pattern="[0-9]*"
                ...
                <div class="text-center">
                    ...
                    <form id="resendForm" method="POST" action="{{ route('admin.resendOtpAdmin') }}" class="d-inline">
                        @csrf
                        <input type="hidden" name="email" value="{{ $email }}">
                        <button id="resendBtn" type="submit" class="btn btn-link p-0 mb-3" disabled>
                            Resend OTP
                        </button>
                    </form>
                </div>
